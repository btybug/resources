<div style="margin-bottom: 200px;" class="popup_div popup_div_{!!$style_id!!}">
       <div class="col-xs-6 col-md-6 left_side">
        @php
            $css= str_replace( ".", " " , "$style_itom->css_data");
            $css = str_replace( "{", " ", "$css");
            $css = str_replace( "}", " ", "$css");
            $css = str_replace( "$style_itom->slug", " ", "$css");
            $css= trim($css);
        @endphp

        {{ HTML::ul($errors->all()) }}

        {{ Form::open(array('method'=>'post', 'url' => '/admin/resources/styles/style_preview/css', 'id'=>'cssedit') )}}
      ...
      <textarea id="css_editor" style="height: 350px; width: 100%; display: none;" name="editor">
       ...
       <div id="editor">{!! $css!!}</div>
        ...
        <input type="hidden" name="styleitom" value="{!! $style_itom->id !!}">
    ...
    <div class="col-xs-6 col-md-6 right_side"><div id="output" class="outputpreview">{!! $style_itom->html !!}</div></div>
